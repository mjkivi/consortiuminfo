	...
	<a name="startcontent" id="startcontent"></a>
		...
		<div class="AllSidebar">
			...
			<?php
				...
				$db = mysql_connect("localhost", "consor5", "c0n$0r667");
				mysql_select_db("consor5_nnews", $db);
				$result = mysql_query("SELECT * FROM items WHERE date2 <> '0000-00-00' ORDER BY date2 ASC LIMIT 1",$db);
				while ($myrow = mysql_fetch_array($result)) {
		...
		<div class="NewsMain">
				...
				<tr>
					<td>
						...
						<?php
							...
							$db = mysql_connect("localhost", "consor5", "c0n$0r667");
							mysql_select_db("consor5_nnews", $db);
							$result = mysql_query("SELECT * FROM items WHERE items.quote <>'' AND date2<=curdate() ORDER BY ID DESC LIMIT 1",$db);
			...
			<table class="NewsBlogEntry">
				...
				<tr>
					<td>
						<?php
							// Standsards Blog Entry
							//echo "\t\t\t\t<p class=\"NewsBlogDate\">".date("F d, Y")."</p>\r";
							//$db = mysql_connect("localhost", "consor5", "c0n$0r667");
							...
							//$EntryMaxLength = 880;
							//$EntryBlurb = $myrow["bodytext"];
							//$EntryBlurb = $myrow["introtext"];
							...
							//}
							//$standardsblogtitle = htmlspecialchars_decode($myrow["title"]);
							//$raw_date = $myrow["date"];
							//$formatted_date = date("F j, Y", strtotime($raw_date));
							//echo "<p style='margin-bottom: 10px;'><strong><a href=\"/standardsblog/article.php?story=".$myrow["sid"]."\">".$standardsblogtitle."</a></strong></p>\r
							//<p><b>Andrew Updegrove</b></p>\r
							//<p><b>Standards Blog - ".$formatted_date.": </b>\r
							//<span>...".strip_tags($EntryBlurb, "<span>")."&hellip;\r
							//<span style=\"color:#001c9d; padding-top:5px;\"><a href=\"/standardsblog/article.php?story=".$myrow["sid"]."\">...Full Story</a></span></span></span></p>\r";
							//<p style=\"color:#001c9d\">&raquo;&nbsp;<a href=\"/standardsblog\">Go to Standards Blog</a></p>\r";
							}
						?>
					</td>
				</tr>
