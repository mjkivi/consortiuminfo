...
$addcat = $_GET["addcat"];
$result = mysql_query("SELECT cons_name FROM cons_list WHERE $addcat=cons_list.cons_id", $db);
...
$result = mysql_query("SELECT  cons_cats.cat_id, cons_cats.cat_name, cons_list.cons_id, cons_list.cons_name, cat_lookup.cat_id, cat_lookup.cons_no FROM ((cons_cats INNER JOIN cat_lookup on cons_cats.cat_id=cat_lookup.cat_id) INNER JOIN cons_list on cons_list.cons_id=cat_lookup.cons_no) WHERE $addcat=cons_no", $db);
...
while ($myrow = mysql_fetch_array($result)) 
	...
	echo "<tr><td><b><font size=2>".$myrow["cat_name"]."</b><br>";
...
?>
				...
				mysql_select_db("consor5_clist", $db);
				$addcat = $_GET["addcat"];
				$result = mysql_query("SELECT cat_id, cat_name FROM cons_cats ORDER BY cat_name ASC", $db);
				while ($myrow = mysql_fetch_array ($result))
				{
				echo "<input name='hidden' type='hidden' value='$addcat'>";
				echo  "<input name='cat_id' type='checkbox' id='cat_id' value=".$myrow["cat_id"].">".$myrow["cat_name"]."<br>";
				}
?> </p>
                                <p>&nbsp;</p>
                                <p>
